...
export function SkillsStep() {
  ...
  // 6) Persist final skills once all pools are spent
  useEffect(() => {
  ...
  const handleSlider = (key, alloc, setter, poolLeft, setPoolLeft) => e => {
    ...
    const prev = alloc[key] || 0, delta = v - prev;
    if (delta <= poolLeft) {
      setter({ ...alloc, [key]: v });
        ...
        <>
          ...
          {cultStd.map(s => (
            <div key={s} className="flex items-center space-x-4 mb-2">
              <div className="w-48 font-medium">{s} (Base {baseStandard[s]}%)</div>
              <input
