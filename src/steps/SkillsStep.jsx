...
export default function SkillsStep({ formData }) {
  ...
  // BaseStandard including parentheses variants
  ...
  // Ensure variants
  new Set([...rawStandard, ...(cultureDef.standardSkills||[]), ...(careerDef.standardSkills||[])]).forEach(name => {
  ...
  // Combat override
  (cultureDef.combatStyles||[]).forEach(style=>baseProfessional[style]=STR+DEX);
  ...
  // Bonus skills list including hobby
  ...
  return <>
    {/* Phase 1 */}
    {phase===1&&<StepWrapper title="Cultural Skills">
      ...
      {cultureDef.standardSkills?.map(s=>{
        ...
        return <div key={s} className="flex items-center mb-2">
      ...
      {cultureDef.professionalSkills?.map(s=><label key={s} className="inline-flex items-center mr-4 mb-2">
        <input type="checkbox" className="mr-1" checked={cProfSel.includes(s)}
          onChange={()=>setCProfSel(sel=>sel.includes(s)?sel.filter(x=>x!==s):sel.length<3?[...sel,s]:sel)} />{s}</label>)}
      {cProfSel.map(s=>{
      ...
      {cultureDef.combatStyles?.map(cs=><label key={cs} className="inline-flex items-center mr-4 mb-2">
        <input type="radio" name="combat" className="mr-1" checked={cCombSel===cs}
          onChange={()=>setCCombSel(cs)} />{cs}</label>)}
      {cCombSel&&<div className="flex items-center mb-2">
        ...
        <input type="range" className="flex-1 mx-2" min={0} max={maxInc} value={cCombAlloc}
          onChange={e=>{const v=Math.min(maxInc,Math.max(0,+e.target.value));const pool=CULT_POOL-sum(cStdAlloc)-sum(cProfAlloc)-cCombAlloc;if(v-cCombAlloc<=pool)setCCombAlloc(v)}}/>
    ...
    {/* Phase 2 */}
    {phase===2&&<StepWrapper title="Career Skills">
      ...
      <h3 className="font-heading text-lg mt-4 mb-2">Professional (max 3)</h3>
      {careerDef.professionalSkills?.map(s=><label key={s} className="inline-flex items-center mr-4 mb-2">
        <input type="checkbox" className="mr-1" checked={rProfSel.includes(s)}
          onChange={()=>setRProfSel(sel=>sel.includes(s)?sel.filter(x=>x!==s):sel.length<3?[...sel,s]:sel)} />{s}</label>)}
      {rProfSel.map(s=>{
    ...
    {/* Phase 3 */}
    {phase===3&&<StepWrapper title="Bonus Skills">
      <p>Bonus left: {bonusLeft}</p>
      {/* Hobby selector */}
      <div className="mb-4">
        <label className="font-medium">Select hobby skill:</label>
        <select value={hobby} onChange={e=>setHobby(e.target.value)} className="ml-2 border rounded p-1">
          <option value="">None</option>
          {[...Object.keys(baseStandard), ...Object.keys(baseProfessional)].map(sk=><option key={sk} value={sk}>{sk}</option>)}
        </select>
      </div>
      {[...bonusSkills].map(s=>{
