...
export default function SkillsStep({ formData, onChange }) {
  ...
  // Build attribute map and computeBase
    ...
    <StepWrapper title="Skills">
      ...
      {phase === 1 && (
        <>
          ...
          <p>Left: <strong>{CULTURAL_POOL - sum(cultStdAlloc) - sum(cultProfAlloc) - cultCombatAlloc}</strong></p>
          <h4>Standard</h4>
          {cultStd.map(s => (
      ...
      {phase === 2 && (
        <>
          ...
          <p>Left: <strong>{CAREER_POOL - sum(carStdAlloc) - sum(carProfAlloc)}</strong></p>
          <h4>Standard</h4>
          {carStd.map(s => (
            <div key={s} className="flex items-center mb-2">
              <span className="w-40">{s} (Base {baseStandard[s]}%)</span>
          ...
          <div className="flex justify-between mt-4">
            <button
              onClick={()=> setPhase(1)} className="px-4 py-2 bg-gray-300 rounded"
            >
              Back
            </button>
            <button
              onClick={()=> setPhase(3)} className="px-4 py-2 bg-gold rounded text-white"
            >
              Next: Bonus
            </button>
          </div>
      ...
      {phase === 3 && (
        <>
          <h3>Bonus Skills</h3>
          <p>Left: <strong>{bonusPoolLeft}</strong></p>
          ...
          <div className="flex justify-between mt-4">
            <button onClick={()=> setPhase(2)} className="px-4 py-2 bg-gray-300 rounded">
              Back
            </button>
            <button onClick={()=> setPhase(4)} disabled={bonusPoolLeft > 0} className="px-4 py-2 bg-green-600 text-white rounded">
              Finish
            </button>
          </div>
        </>
